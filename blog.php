...
<body>
    ...
    function communityCreation($bdd, $id){
        ...
        while($count2 < count($data2)){
            ...
            echo "<div style='border-radius: 24px; border: 3px solid rgb(0 140 2)'><a href='afficheArticle.php?id=$article_id2'><h3>$title2</h3><h5>Créé le : $createdDate2 par $author, Dernière modification : $update</h5></a></div><br>";
            $count2 += 1;
        }
    }

    if(!empty($_SESSION) && !empty($_SESSION["email"])) { // Faire une requête qui récupère tous les articles qu'a fait l'utilisateur et afficher en dessous
        ...
        try {
            ...
            while($row < count($data)){
                ...
                echo "<div style='border-radius: 24px; border: 3px solid rgb(0 142 2)'><a href='afficheArticle.php?id=$article_id'><h3>$title</h3><h5>Créé le : $createdDate par Vous, Dernière modification : $update</h5></a></div>";
            ...
            if($e->getCode() == 23000 ) { // Le code 23000 correspond à une entrée dupliquée ça veux dire que l'adresse mail est déjà en bdd
